class PluralizeSwedish::Inflections < ::ActiveSupport::Inflector::Inflections
  class << self
    def inflections_plural(i)
      # declination 1 -or
    ...
    def inflections_uncountable(i)
      i.uncountable %w(hus kar träd får brev namn nummer kön)
    end

    def inflections_irregular(i)
    end
    ...
    def pluralize(word)
      ...
      if inflections.uncountables.include?(result.mb_chars.downcase.to_s)
        result
      else
